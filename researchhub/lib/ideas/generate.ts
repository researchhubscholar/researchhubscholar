...
export type Idea = BaseIdea & { justification: string; unresolved: string[]; plan: string[]; references: Reference[]; contextSummary: string; startingQuestion: string };
...
export function generate(context: Context, evidence: Evidence[] = []): Idea[] {
  ...
  return generateBase(context).map(idea => {
    ...
    const unresolved = [
      ...
      ...(review ? ["Teste uma busca piloto, confirme acesso aos textos e escolha como avaliar criticamente os desenhos elegíveis."] : ["Estime a amostra viável e confira autorizações, avaliação ética aplicável e disponibilidade de apoio para a análise."]),
...
export function ideaBrief(idea: Idea): string {
  return ["PROPOSTA PARA DISCUSSÃO COM O ORIENTADOR", idea.title, "", "CONTEXTO", idea.contextSummary,
    ...
    "", "JUSTIFICATIVA INICIAL", idea.justification, "", "DESFECHO", idea.outcome, "", "MÉTODOS", idea.methods, "", "ANÁLISE", idea.analysis,
    "", "VIABILIDADE", idea.feasibility, "", "DECISÕES PENDENTES", ...idea.unresolved.map(value => `• ${value}`),
