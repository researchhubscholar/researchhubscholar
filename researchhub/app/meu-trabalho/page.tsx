...
export default function MeuTrabalhoPage() {
    ...
    async function load() {
      ...
      try {
        ...
        if (auth.user && !isNew) {
          ...
          if (data) {
            setDraft(fromRecord(data)); setProjectId(data.id); setProjectStatus(data.status || "draft"); setSaveState("saved");
  ...
  function applyIncoming() {
    if (!incoming) return;
    setDraft(incoming); setProjectId(null); setProjectStatus("draft"); setIncoming(null); setDirty(true); setSaveState("idle");
  ...
  async function saveDraft() {
    ...
    try { window.localStorage.setItem(storageKey, JSON.stringify(draft)); localSaved = true; } catch { /* O salvamento na conta continua disponível. */ }
    try {
      const db = supabaseBrowser();
      const { data: auth, error: authError } = await db.auth.getUser();
      ...
      setProjectId(data.id); setLoggedIn(true); setSaveState("saved"); setDirty(false);
      setMessage("Projeto salvo na sua conta. Você pode abri-lo em Meu espaço.");
      window.history.replaceState(null, "", `/meu-trabalho?id=${encodeURIComponent(data.id)}`);
      try { window.localStorage.removeItem(storageKey); } catch { /* A cópia na conta foi confirmada. */ }
    } catch {
      setSaveState(localSaved ? "local" : "error"); setDirty(!localSaved);
