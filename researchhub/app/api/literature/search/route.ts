...
export async function POST(request: NextRequest) {
 ...
 if(topic.length<3||topic.length>300||!["all","3","5","10"].includes(period)||!["all","systematic","trial","observational","review","case"].includes(studyType))return NextResponse.json({error:"Confira o tema e os filtros."},{status:400});
