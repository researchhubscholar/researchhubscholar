  ...
  return <div className="max-w-5xl mx-auto">
    ...
    <form onSubmit={e => { e.preventDefault(); setDiagnosisVisible(true); explore(context); }} className="mt-8 bg-white border border-line rounded-2xl p-5 md:p-6">
      ...
      <div className="grid md:grid-cols-2 gap-4">
        <Select label="Que tipo de trabalho você precisa desenvolver?" value={context.workType || "open"} change={v => update("workType", v)} options={[["open", "Quero comparar caminhos"], ["tcc", "TCC ou trabalho de conclusão"], ["original", "Artigo ou estudo original"], ["review", "Revisão de literatura"], ["case", "Relato de caso"], ["residency", "Projeto da residência"]]} />
    ...
    {ideas.length > 0 && <section className="mt-8" aria-label="Ideias sugeridas">
      ...
      <div className="grid md:grid-cols-2 gap-5 mt-5">{ideas.map((idea, index) => <article key={idea.id} className="bg-white border border-line rounded-2xl p-5 md:p-6 flex flex-col">
        ...
        {idea.evaluation && <div className="grid grid-cols-3 gap-2 mt-5" aria-label="Avaliação deste caminho">
          {([ ["Relevância", idea.evaluation.relevance], ["Viabilidade", idea.evaluation.feasibility], ["Execução", idea.evaluation.execution] ] as const).map(([label, item]) => <div key={label} className="border border-line rounded-card p-3 text-center"><span className="block text-xl font-semibold text-teal">{item.score}</span><span className="block text-xs font-medium mt-1">{label}</span><span className="block text-[11px] text-ink-soft mt-1">{item.label}</span></div>)}
        </div>}
        ...
        {idea.evaluation && <details className="mt-4 text-sm"><summary className="text-teal font-medium cursor-pointer">Entender as pontuações</summary><dl className="mt-3 space-y-3">{([ ["Relevância", idea.evaluation.relevance], ["Viabilidade", idea.evaluation.feasibility], ["Execução", idea.evaluation.execution] ] as const).map(([label, item]) => <div key={label}><dt className="font-medium">{label} · {item.score}/100</dt><dd className="text-ink-soft mt-1">{item.reason}</dd></div>)}</dl><p className="text-xs text-ink-soft mt-3">A pontuação organiza as condições informadas; não substitui avaliação metodológica ou ética.</p></details>}
        {idea.refinements?.length > 0 && <div className="mt-5 border-l-2 border-teal pl-4 text-sm"><p className="font-medium">Como deixar esta ideia mais pesquisável</p><ul className="mt-2 space-y-2 text-ink-soft">{idea.refinements.map(value => <li key={value}>• {value}</li>)}</ul></div>}
    ...
    {selected.length >= 2 && <section className="mt-8 bg-white border border-line rounded-2xl p-5"><h2 className="font-display text-2xl">Compare antes de escolher</h2><p className="text-sm text-ink-soft mt-2">Use as notas como apoio para enxergar diferenças. A melhor proposta é a que combina relevância com condições reais de execução.</p><div className="overflow-x-auto mt-4"><table className="w-full text-sm text-left"><caption className="sr-only">Comparação das ideias selecionadas</caption><thead><tr><th scope="col" className="p-3">Critério</th>{ideas.filter(x => selected.includes(x.id)).map(x => <th scope="col" key={x.id} className="p-3 min-w-60">{x.title}</th>)}</tr></thead><tbody><tr className="border-t border-line bg-teal-soft"><th scope="row" className="p-3 align-top">Pontuação geral</th>{ideas.filter(x => selected.includes(x.id)).map(x => <td key={x.id} className="p-3 align-top font-semibold text-teal">{x.evaluation?.overall ?? "—"}/100</td>)}</tr>{([ ["Desenho", "studyType"], ["Pergunta", "question"], ["Objetivo", "objective"], ["População", "population"], ["Desfecho", "outcome"], ["Métodos", "methods"], ["Recursos", "resources"], ["Prazo e acesso", "contextSummary"], ["Viabilidade", "feasibility"], ["Dificuldades", "difficulty"], ["Justificativa", "justification"] ] as const).map(([label, key]) => <tr key={key} className="border-t border-line"><th scope="row" className="p-3 align-top">{label}</th>{ideas.filter(x => selected.includes(x.id)).map(x => <td key={x.id} className="p-3 align-top text-ink-soft">{x[key]}</td>)}</tr>)}</tbody></table></div></section>}
