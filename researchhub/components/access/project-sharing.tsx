...
export default function ProjectSharing({projectId}:{projectId:string|null}){
 ...
 useEffect(()=>{const epochRef=epoch;const ticket=++epochRef.current;setOptions([]);setShares([]);setMessage("");if(!projectId)return;const client=supabaseBrowser();
  void(async()=>{try{const {data:auth}=await client.auth.getUser();if(!auth.user||ticket!==epochRef.current)return;owner.current=auth.user.id;
   const {data:own}=await client.from("research_projects").select("owner_id").eq("id",projectId).eq("owner_id",auth.user.id).maybeSingle();if(!own||ticket!==epochRef.current)return;
   ...
   if(ticket!==epochRef.current)return;if(orgs.error||links.error)return;setOptions(orgs.data||[]);setShares((links.data||[]).map(s=>s.organization_id));
  }catch{}})();
  const {data:listener}=client.auth.onAuthStateChange((event,session)=>{if(event==="SIGNED_OUT"||(event==="SIGNED_IN"&&owner.current&&session?.user.id!==owner.current)){++epochRef.current;setOptions([]);setShares([]);owner.current=null;}});
  return()=>{++epochRef.current;listener.subscription.unsubscribe();};
 },[projectId]);
